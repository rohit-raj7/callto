...
class _ChatPageState extends State<ChatPage> with WidgetsBindingObserver {
  ...
  Future<void> _loadMessagesFromApi() async {
    ...
    if (!_historyReceived && mounted) {
      print(
        '[ChatPage-Listener] Socket history not received, loading from API',
      );
      try {
  ...
  void _setupSocketListeners() {
    ...
    _messageSubscription = _socketService.onChatMessage.listen((data) {
      ...
      if (chatId == _chatId && data['message'] != null) {
        ...
        print(
          '[ChatPage-Listener] Received message from socket: ${message.messageContent} (senderId: ${message.senderId})',
        );
        print(
          '[ChatPage-Listener] TIMESTAMP DEBUG: raw=${messageData['created_at']}, parsed=${message.createdAt}, isUtc=${message.createdAt?.isUtc}, local=${message.createdAt?.toLocal()}',
        );
        ...
        setState(() {
          ...
          if (message.senderId == _currentUserId) {
            ...
            if (optimisticIndex != -1) {
              print(
                '[ChatPage-Listener] Replacing optimistic message with confirmed message',
              );
          ...
          if (!_messages.any((m) => m.messageId == message.messageId)) {
            ...
            print(
              '[ChatPage-Listener] Message already exists, skipping duplicate',
            );
          }
        });
        _scrollToBottom();
    ...
    _historySubscription = _socketService.onChatHistory.listen((data) {
      ...
      if (chatId == _chatId && data['messages'] != null) {
        ...
        print(
          '[ChatPage-Listener] Received history: ${messagesList.length} messages',
        );
    ...
    _readSubscription = _socketService.onChatMessagesRead.listen((data) {
      ...
      if (chatId == _chatId) {
        ...
        print(
          '[ChatPage-Listener] Messages marked as read by ${data['readBy']}',
        );
      }
    });
